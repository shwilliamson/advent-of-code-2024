...
def dir_symbol(direction: Callable[[int, int], Tuple[int, int]], matrix: [str], i: int, j: int) -> Optional[str]:
    ii, jj = direction(i, j)
...
def x_mas_count(matrix: [str], i: int, j: int,) -> int:
    if matrix[i][j] != 'A':
        return 0
    symbols = { key : dir_symbol(dir, matrix, i, j) for key, dir in dirs.items() }
